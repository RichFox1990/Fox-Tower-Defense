...
class Tower(pg.sprite.Sprite):
    def __init__(self, game: 'Game'):
        ...
        self.rect = pg.Rect(0, 0, TILE_SIZE*TOWER_MULTIPLIER,
                            TILE_SIZE)  # for collisions
    ...
    def prep_radius(self):
        ...
        pg.draw.circle(self.rad_surface,
                       COLOURS["gray"], self.rad_rect.center, self.radius)
        pg.draw.circle(self.rad_surface,
                       COLOURS["white"], self.rad_rect.center, self.radius, 1)
        self.rad_surface.set_colorkey(COLOURS["magenta"])
        self.rad_surface.set_alpha(50)
        self.rad_rect.center = self.rect.center

    # Function thats passed initially and when the tower updates to set the attack platform images of the tower and position them correctly. (TODO: tweak a better, less hard coded formula)
    ...
    def update_attack_images(self):
        if self.platform:
            ...
            self.f_rect = self.front.get_rect(
                midtop=self.image_rect.center + self.f_offset)  # front section rect
            self.b_rect = self.back.get_rect(
                midbottom=self.f_rect.midtop + self.b_offset)  # back section rect
    ...
    def setup_animation_values(self):
        if self.platform:
            # distance in pixels how much we have to move to hit the top
            direction_vec = Vec(self.b_rect.top - self.image_rect.top)
            pixels = direction_vec.length()

            # total of pixels to move the platform up from the bottom to the top of the animation ( *.7 = only using 70% of the distance)
            self.pixels = int(abs(pixels)*.8)
    ...
    def animate(self):
        # 0-1 measures how far the attack_timer is towards the end of the timer with a value 0-1 ( 0.5 = 50% , 1 = 100% etc)
        self.amount = self.attack_timer/self.attack_animation_time

        # front section rect move rect by total pixels multiplied by the progress of the timer to 100%
        self.f_rect.midtop = self.orig_f_rect.midtop + \
            Vec(0, -(self.pixels*(self.amount)))
        # matches the back platform rect to the needed position dependant on the front rects ( + correction Vec)
    ...
    def update(self, dt):
        ...
        #	self.image = self.tower_images[self.level-1]
        if self.placed:
            # handles the shooting platform animation timer and cooldown (this function also attacks the relevant mob when timer at the correct point)
            self.handle_attack_timers(dt)
            if self.platform:
                self.animate()
            ...
            if self.attack_timer == 0 and self.cooldown_timer == 0:
                ...
                if self.target != False:
                    # add to the delay variable to begin a delay to animate the shooting platform
                    self.attack_timer = 1
                    # add to the delay variable to begin a cooldown timer (which is equal to the attack timer + the cooldown before it can shoot again)
    ...
    def handle_upgrade(self):
        if self.max_level_reached():
            print("MAX LEVEL REACHED")
            return False

        if self.cost[self.level] <= self.game.money:
            ...
            self.image = self.tower_images[self.level-1]
            self.image_rect = self.image.get_rect(
                midbottom=self.image_rect.midbottom)
            return True
    ...
    # This is called when the tower is selected to be purchased, handles the image following the mouse (on a grid format) and whether it can be purchased when the user clicks
    # if it can it sets the relevant values and calls the relevant functions needed to initiate the tower
    def handle_purchase(self):
        if self.game.building_tower != False:
            # Grid position
            self.tower_sq_pos = (
                (self.game.mpos[0]//TILE_SIZE) * TILE_SIZE, (self.game.mpos[1]//TILE_SIZE) * TILE_SIZE)
            self.tower_pos = pg.rect.Rect(
                self.tower_sq_pos[0], self.tower_sq_pos[1], TILE_SIZE*TOWER_MULTIPLIER, TILE_SIZE)

            self.rect = self.tower_pos
            self.image_rect.midbottom = self.rect.midbottom
            self.h_rect = self.highlight.get_rect(
            ...
            if self.game.click:
                ...
                if len(self.game.towers) > 1:
                    for tower in self.game.towers:
                        # Check new tower doesn't collide (excluding colliding with itself)
                ...
                if len(collide_list) == 0:
                    ...
                    if cost <= self.game.money:
                        ...
                        # pg.Rect(tower_sq_pos[0], tower_sq_pos[1], TILE_SIZE, TILE_SIZE)
                        self.image_rect.midbottom = self.rect.midbottom
                        self.menu = TowerMenu(self.game, self)
                        self.level_up_values()
                    else:
                        print(
                            f"you cannot afford this tower. You need {cost} coins")
    ...
    def check_targets(self):
        ...
        if len(self.targets) != 0:
            ...
            if f_m == "HQ":
                self.targets.sort(
                    key=lambda x: (-x.turn_count, x.dist_from_target))
            elif f_m == "Low HP":
    ...
    def draw_overlays(self, screen):
        pg.draw.circle(screen, COLOURS["gray"],
                       self.rect.center, self.radius, 3)
        if len(self.targets) != 0:
            for target in self.targets:
                pg.draw.line(
                    screen, COLOURS["lightblue"], self.rect.center, target.rect.center, 4)
            pg.draw.line(
                screen, COLOURS["red"], self.rect.center, self.targets[0].rect.center, 4)
    ...
    def draw(self, screen):
        ...
        if self.selected:
            ...
            if self.game.building_tower != False:
                pg.draw.circle(
                    screen, COLOURS["gray"], self.rect.center, self.radius, 3)
                self.draw_tower(screen)
