...
class Coins(pg.sprite.Sprite):
    def __init__(self, game: 'Game', entity, spread):

        self.game = game
        self.groups = self.game.coins, self.game.all_sprites
        pg.sprite.Sprite.__init__(self, self.groups)

        # Random spread value in pixels the coin can spawn from the entity's center point
        self.spread = spread
        self.entity = entity
        # list of images used to animate the coin
        self.images = self.game.images["coin"]
        self.image_number = 0
    ...
    def prep_shadow(self):
        ...
        pg.draw.ellipse(
            self.shadow_surface, COLOURS["black"], (0, 0, self.width*.8, self.shadow_height))
    ...
    def randomize_pos(self):
        randomx, randomy = rand.randrange(-self.spread,
                                          self.spread), rand.randrange(-self.spread, self.spread)
