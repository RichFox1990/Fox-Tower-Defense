...
# make this a parent for multiple types of projectile
class Projectile(pg.sprite.Sprite):
    def __init__(self, game: 'Game', tower):
        ...
        if self.firing_type == "splash":
            # Time in seconds it takes for the shot to land
            self.shot_total = PROJECTILE_SPEED
        ...
        if self.tower.platform:
            self.rect = self.image.get_rect(
                midbottom=self.tower.b_rect.midbottom)
            self.pos = Vec(self.rect.midbottom)
        else:
            self.rect = self.image.get_rect(
                center=self.tower.image_rect.midtop)
            self.pos = Vec(self.rect.center)
        # height the projection arcs into the air towards target
        self.loop_speed = self.tower.loop_height
    ...
    def set_target(self, target):
        # used to reference when moving the projectile in "move" method
        self.orig_rect = self.rect.copy()
        self.target = target
        self.target_pos = target.rect.center  # + Vec(0, -5)

        # position i want the projectile to hit (midbottom of mob (the floor at mobs
        t_pos = Vec(self.target_pos)
        # feet))
        # want to use the center of projectile to match with the above
        my_pos = Vec(self.pos)
    ...
    def move(self, dt):
        # gets a value between 0 and 1 representing how far through the shot we are (i.e 0.5 would equal 50%)
        stage_of_shot = self.shot_timer/self.shot_total

        if self.firing_type == "splash":

            if not self.hit:
                self.pos = self.orig_rect.midbottom + \
                    Vec(self.t_vec * stage_of_shot)
            ...
            try:
                ...
                full_dist = Vec(Vec(self.target.rect.center) -
                                Vec(self.orig_rect.center))
            ...
            # self.orig_rect.midbottom + Vec(self.t_vec * stage_of_shot)
            self.pos += self.vel
            self.rect.center = self.pos
        ...
        if self.firing_type == "splash":
            ...
            if now - self.update_hit > PARTICLE_ANIMATION_SPEED:
                if self.image_number != len(self.images):
                    self.image_number += 1
                    self.image = self.images[self.image_number % len(
                        self.images)-1]
    ...
    def particles(self, amount, offset=0):
        if self.tower.name == "Fire":
            ...
            for iteration in range(amount):
                ...
                self.tracers.append([particle_pos, Vec(rand.randint(0, 20) / 10 - 1, rand.randint(
                    0, 20) / 10), rand.randint(1, 2), col_list[rand.randint(0, len(col_list)-1)]])
    ...
    def draw(self, screen):
        if self.shot:
            ...
            for particle in self.tracers:
                pg.draw.circle(screen, particle[3], [
