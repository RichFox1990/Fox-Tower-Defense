...
class Mob(pg.sprite.Sprite):
    def __init__(self, game: 'Game'):
        ...
        # to account for using the center postion to blit (makes the mobs midbottom appear where the path offset was chosen)
        self.pos -= Vec(self.height, self.height)/2

        # now set the rect we use to draw the image equal to the position
        ...
        self.health_rect = self.health_images[0].get_rect(
            center=self.rect.midtop + Vec(0, -5))
    ...
    def prep_shadow(self):
        ...
        self.shadow_rect = self.shadow_surface.get_rect(
            midbottom=self.rect.midbottom - Vec(0, self.shadow_height))
        pg.draw.ellipse(self.shadow_surface,
                        COLOURS["black"], (0, 0, self.width, self.shadow_height))
    ...
    def handle_health(self, tower_damage):
        if self.health - tower_damage > 0:
            ...
            self.health = 0
        self.health_percentage = self.health / self.max_health
        self.health_images[1] = pg.transform.scale(self.health_images[1], (int(
            self.health_original_width * self.health_percentage), self.health_original_height))
        if self.health == 0:
            self.alive = False
            self.death_delay = 1
    ...
    def follow_target(self, target, dt):
        ...
        if self.target_direction == "x":
            self.desired = Vec(
                Vec(target + self.offset, self.pos.y) - self.pos)
            self.dist_from_target = abs(self.desired.x - self.offset)

        if self.target_direction == "y":
            self.desired = Vec(
                Vec(self.pos.x, target + self.offset - self.height/1.5) - self.pos)
            self.dist_from_target = abs(
                self.desired.y - self.offset + self.height/1.5)
    ...
    def draw_vectors(self, screen):
        ...
        pg.draw.line(screen, self.colour, self.pos,
                     (self.pos + self.desired), 5)
